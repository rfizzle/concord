...
| **[Respite](https://respite.rfizzle.com)** | Sleep, time & vitality | "Make the night count." | Beta |
| **[Distillation](https://distillation.rfizzle.com)** | Brewing & effects | "Every drop counts." | Beta |
| **[Cultivation](https://cultivation.rfizzle.com)** | Farming & food | "Worth growing." | Beta |
| **[Instinct](https://instinct.rfizzle.com)** | Animals & husbandry | "Worth raising." | Beta |
...
every member mod conforms to, and the collection landing site (content in `site/`,
built by the shared template). Mod repos **link to these documents — they never copy them.**
...
| [`members.json`](members.json) | The member registry — per-member `status`, `conformance` (layout migration), name/tagline/url, and `store` (Modrinth/CurseForge project id + slug); drives every site's cross-mod footer and the propagate workflow. `status` is `released` once a **stable** `1.x` tag exists; a member whose only tags are prereleases (a published beta) stays `in-development`. The `store` ids are the canonical source for the publish (`modrinth-id`/`curseforge-id`) and listing-sync workflow inputs |
...
| [`propagate/`](propagate) | Canonical concord-owned files (currently `versions-common.properties`, the suite toolchain pins) proposed verbatim to every member repo via a `concord-sync` PR by `propagate.yml` (member default branches are protected) — edit HERE, never in a mod repo |
...
| [`.ai/commands/`](.ai/commands) | Canonical slash commands (`/glyph`, `/sfx`, `/assess`, `/align`, `/conform`, `/implement`) for all member repos — vendored by the same `make sync` target, surfaced to Claude Code via a `.claude/commands` → `.ai/commands` symlink. Edit HERE, never in a mod repo |
...
skipped where it doesn't. Secrets per repo: `CLAUDE_CODE_OAUTH_TOKEN` (the Claude
workflows), `MODRINTH_TOKEN` (`mod-release`, `mod-listing-sync`), and `CURSEFORGE_TOKEN`
(`mod-release`) — see [`design/REPO-SETTINGS.md`](design/REPO-SETTINGS.md) §4.
