...
- Favicon: the template ships none today and no member wires one; three members
  (tribulation, prosperity, cultivation) carry a vestigial `apple-touch-icon.png` that
  nothing references. Treat a favicon set as unsupported until the template grows it —
  do not add per-member icons in the meantime.
- `ogImage` (site.json, optional): the social-card image, defaulting to `logo.png`
...
**Companion `.glyph` files (repeatability).** Every committed texture master carries a
`.glyph` source. Specs live under **`art/glyphs/<name>.glyph`** — or beside the master,
where a mod prefers that; both are conformant — and the shipped PNG lives only under
`src/main/resources/assets/<mod>/`, with `design/ASSETS.md` as the authoritative join
between spec and shipped path. A size ladder commits one `.glyph` per natively-authored
tier. The `.glyph` is the source of truth: minor edits
...
**What every member's `art/` carries.** `logo.png` (the §4 masthead master),
`icon-128.png` and `icon-512.png` (the store/launcher masters — 8/8 ship both),
`glyphs/*.glyph` specs, `audio/*.sfx` specs where the mod ships sound, and
`exploration/` for generation prompts. A spec generated by a script commits that
script beside it as `<name>.gen.py`, so the generated spec is itself reproducible.
...
**Companion `.sfx` files (repeatability).** Every committed sound carries its `.sfx`
spec in the mod's **`art/audio/`**. The spec is the only audio master committed there;
the rendered `.ogg` is derived and lives only in `assets/<mod>/sounds/`, joined to its
spec by `design/ASSETS.md`. The `.sfx` is the source of truth: edits re-render in seconds
and the sound is reproducible from the spec alone. Re-touching a sound
...
| `subtitles.<mod>.*` | Sound subtitles — required for every custom sound (§9) | `subtitles.tribulation.blood_moon` |
...
| `config.<mod>.<field>` labels and their `.tooltip` pairs | `camelCase` | The key mirrors the Java config field it labels, so field and key stay mechanically aligned. The last segment is the Java field name; a nested section segment (`config.meridian.enchantingTable.maxEterna`) is allowed and a flat key (`config.prosperity.hudAnchor`) is too — whichever a mod shipped stays. [`HUD-STANDARD.md`](../HUD-STANDARD.md) §4 already fixes `hudAnchor` / `hudOffsetX` / `hudOffsetY` on exactly this basis. |
...
Enum *values* inside a key stay snake_case wherever they name a state
(`hud.tribulation.tier.blood_moon`), matching every other authored surface.

⚠️ **Lang keys are visible to resource packs.** A member correcting its casing is
making a player-facing change: batch it into a minor release with a changelog note,
never a patch.
