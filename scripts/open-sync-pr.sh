...
# 3) .gitignore region — same treatment as AGENTS.md (markers in hash-comment form).
...
if [ -z "$existing" ]; then
  gh pr create -R "$REPO" --base "$DEFAULT" --head "$BRANCH" \
    ...
    --body "Automated sync from concord@$SHORT (issue templates, AGENTS.md + .gitignore shared regions). Generated by propagate.yml — merge to adopt; the branch refreshes on each concord change."
  echo "$REPO: opened PR"
else
