...
if [ -z "$existing" ]; then
  gh pr create -R "$REPO" --base "$DEFAULT" --head "$BRANCH" \
    ...
    --body "Automated sync from concord@$SHORT (issue templates, versions-common.properties suite toolchain pins, AGENTS.md + .gitignore shared regions, vendored .ai/skills + .ai/commands). Generated by propagate.yml — merge to adopt; the branch refreshes on each concord change."
  echo "$REPO: opened PR"
else
